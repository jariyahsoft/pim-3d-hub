...
export function createMoneyMinor(minorUnits: number, currency: string): MoneyMinor {
...
export function parseMillimeter(value: number): Millimeter {
  if (!Number.isFinite(value) || value <= 0) {
    throw new RangeError('Dimension values must be finite and greater than zero')
  }
