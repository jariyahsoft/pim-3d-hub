...
export function createInMemoryFileScanResultRepository(input?: {
  clock?: ClockPort;
  uuidGenerator?: UuidGeneratorPort;
...
export function createInMemoryFileRetentionHoldRepository(input?: {
  clock?: ClockPort;
  uuidGenerator?: UuidGeneratorPort;
  ...
  function requireRecord(id: Uuidv7): FileRetentionHoldRecord {
    ...
    if (!record) {
      throw new Error(`FileRetentionHold ${id} was not found`);
    }
    return record;
  }
    ...
    repository: Object.freeze({
      ...
      async update(
        ...
        if (current.version !== expectedVersion) {
          // Reuse a permissive unique-constraint error to keep the boundary
          // simple - the application layer wraps the cause as a conflict.
          throw new RepositoryUniqueConstraintError({
